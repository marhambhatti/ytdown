...
def ensure_cookie_file_from_env():
    ...
    try:
        if cookies_b64:
            cookies_text = base64.b64decode(cookies_b64).decode('utf-8')
        if not cookie_content_is_valid(cookies_text):
            print("Cookie env invalid/corrupt — skip.", flush=True)
            return
        write_cookie_file(cookies_text)
    except Exception as e:
        print(f"Cookie env load failed: {e}", flush=True)
...
def cookie_diagnostics():
    if not cookies_available():
        return {"status": "not found", "has_youtube_domain": False, "auth_cookie_names": []}
    found_names = set()
    has_youtube_domain = False
    try:
        ...
        return {"status": "unreadable", "has_youtube_domain": False, "auth_cookie_names": []}
    return {"status": "loaded", "has_youtube_domain": has_youtube_domain, "auth_cookie_names": sorted(found_names)}
...
# ✅ KEY FIX: skip_manifests=False — HLS/DASH formats include karo taake sab qualities milein
def make_info_opts():
    opts = {
        'quiet': True,
        'no_warnings': True,
        'skip_download': True,
        # ✅ extractor_args bilkul nahi — YouTube apne aap sab formats dega
    }
    opts.update(cookie_opts())
    apply_yt_dlp_runtime(opts)
    return opts
...
def cleanup_old_jobs():
    while True:
        time.sleep(300)
        now = time.time()
        expired = [jid for jid in list(jobs.keys()) if now - jobs[jid].get('created_at', now) > 1800]
        for jid in expired:
            try:
                output_path = jobs[jid].get('output_path') or f'downloads/{jid}'
                if os.path.exists(output_path):
...
def update_cookies():
    ...
    try:
        ...
        return jsonify({"error": f"Cookies save nahi hui: {e}"}), 500
    return jsonify({"success": True, "path": COOKIE_FILE, "size": os.path.getsize(COOKIE_FILE)})
...
def format_view_count(count):
    ...
    if count >= 1_000_000:
        return f"{count/1e6:.1f}M".replace(".0M", "M")
    if count >= 1_000:
        return f"{count/1e3:.1f}K".replace(".0K", "K")
    return str(count)
...
def estimate_filesize(quality):
    return {
        "8K": "~3.0 GB", "4K": "~800 MB", "2K": "~400 MB",
        "1080p": "~120 MB", "720p": "~70 MB", "480p": "~40 MB",
        "360p": "~25 MB", "240p": "~15 MB", "144p": "~8 MB",
        "Audio Only": "~5 MB",
    }.get(quality, "~50 MB")


def height_to_quality_label(height):
    """
    ✅ FIX: Nearest standard quality label return karo.
    YouTube kabhi 1088, 1086 jesi non-standard heights deta hai.
    Breakpoints: agar height >= threshold toh us label ko use karo.
    """
    if not height:
        return None
    breakpoints = [
        (3600, "8K"),   # 4320 nahi karte — kuch servers 3840 dete hain
        (1900, "4K"),   # 2160 ki jagah
        (1300, "2K"),   # 1440 ki jagah
        (900,  "1080p"),# 1080/1088 dono cover
        (620,  "720p"), # 720 cover
        (420,  "480p"), # 480 cover
        (300,  "360p"), # 360 cover
        (200,  "240p"), # 240 cover
...
def get_format_list(formats):
    ...
    # ✅ Video formats — sab heights process karo
    for fmt in formats:
        vcodec = fmt.get('vcodec') or ''
        acodec = fmt.get('acodec') or ''
        height = fmt.get('height')

        # Skip audio-only aur invalid formats
        if vcodec in ('none', '') or not height:
            continue

        label = height_to_quality_label(height)
        if not label:
            continue

        existing = quality_map.get(label)
        # Higher actual height prefer karo (e.g. 1088 over 1080 for same label)
        if not existing or height > existing.get('_height', 0):
            filesize = fmt.get('filesize') or fmt.get('filesize_approx')
            quality_map[label] = {
                "quality": label,
                "_height": height,          # internal use only
                "format_id": fmt.get('format_id', ''),
                "ext": fmt.get('ext', 'mp4'),
                "filesize": format_filesize(filesize) if filesize else estimate_filesize(label),
            }
    ...
    quality_order = ["8K", "4K", "2K", "1080p", "720p", "480p", "360p", "240p", "144p", "Audio Only"]

    result = []
    for q in quality_order:
        if q in quality_map:
            entry = dict(quality_map[q])
            entry.pop('_height', None)   # internal field remove karo
            result.append(entry)
...
def yt_dlp_error_message(error):
    msg = str(error).lower()
    if "sign in to confirm" in msg or "not a bot" in msg or "use --cookies" in msg:
        if cookies_available():
            return "YouTube ne cookies accept nahi ki. Fresh cookies export karke dobara upload karein."
        return "YouTube login cookies required hain."
    if "n challenge solving failed" in msg or "no video formats found" in msg:
        return "YouTube formats fetch nahi ho sake. App redeploy karein aur phir try karein."
    if "requested format is not available" in msg:
        return "Is video ke formats available nahi hain."
    if "private" in msg:
        return "Yeh video private hai"
    if "age" in msg:
        return "Age restricted video — login required"
...
def build_ydl_opts(job_id, quality, fmt, output_path):
    ...
    if is_audio_only:
        ...
        max_height = quality_heights.get(selected_quality, 720)
        # ✅ Height exact match ke saath +10% tolerance — 1088 bhi 1080p request pe milega
        max_height_tolerant = int(max_height * 1.10)
        format_str = (
            f"bestvideo[height<={max_height_tolerant}][ext=mp4]+bestaudio[ext=m4a]/"
            f"bestvideo[height<={max_height_tolerant}]+bestaudio/"
            f"best[height<={max_height_tolerant}]/"
            "best"
        )
...
def run_download(job_id, url, opts):
    try:
        ...
        t = threading.Thread(target=cleanup_download_folder, args=(job_id,))
        t.daemon = True
        t.start()
...
def get_downloaded_files(output_path):
    if not os.path.exists(output_path):
        return []
    skip_exts = {'.part', '.ytdl', '.temp', '.tmp', '.zip'}
    files = []
    for name in os.listdir(output_path):
        path = os.path.join(output_path, name)
        _, ext = os.path.splitext(name)
        if os.path.isfile(path) and ext.lower() not in skip_exts:
            files.append(path)
    return files
