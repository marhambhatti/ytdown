...
# ✅ FIX: Height ko nearest standard quality pe map karo
def height_to_quality_label(height):
    """
    YouTube har video ke liye exact height nahi deta (e.g. 1088 instead of 1080).
    Is function mein nearest standard quality label return hota hai.
    """
    if height is None:
        return None

    # Standard breakpoints — upar wali range tak match karo
    breakpoints = [
        (4320, "8K"),
        (2160, "4K"),
        (1440, "2K"),
...
def get_format_list(formats):
    ...
    # ✅ FIX: Har height ko quality label pe map karo, phir best format rakho per label
    quality_map = {}  # label -> best format dict
    ...
    for fmt in video_formats:
        ...
        # Agar pehle se hai toh higher height wala prefer karo
        existing = quality_map.get(label)
        if not existing or height > existing.get('height', 0):
            filesize = fmt.get('filesize') or fmt.get('filesize_approx')
            quality_map[label] = {
                "quality": label,
                "height": height,
    ...
    clean_formats = sorted(
        quality_map.values(),
        key=lambda x: quality_order.index(x["quality"]) if x["quality"] in quality_order else 99
    )
...
def build_ydl_opts(job_id, quality, fmt, output_path):
    # ✅ FIX: 240p aur 144p bhi support karo
    quality_heights = {
        "8K":    4320,
        "4K":    2160,
        "2K":    1440,
